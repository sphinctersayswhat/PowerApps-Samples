...
namespace PowerApps.Samples
{
    class Program
    {
        public static Uri BaseAddress { get { return client?.BaseAddress; } }
        ...
        static void Main()
        {
            ...
            string resource = "https://ttc-erm-sandbox.api.crm9.dynamics.com";
            ...
            var contoso = Get($"{account1Uri}?$select=accountid,name");
            ...
            JToken containsSampleinFullNameCollection = Get("contacts?" +
            ...
            JToken createdInLastHourCollection = Get("contacts?" +
            "$select=fullname,jobtitle,annualincome,emailaddress1&" +
            ...
            JToken highIncomeContacts = Get("contacts?" +
                "$select=fullname,jobtitle,annualincome&" +
                "$filter=contains(fullname,'(sample)') and " +
            ...
            JToken seniorOrSpecialistsCollection = Get("contacts?" +
                "$select=fullname,jobtitle,annualincome&" +
                "$filter=contains(fullname,'(sample)') and " +
            ...
            JToken orderedResults = Get("contacts?" +
                "$select=fullname,jobtitle,annualincome&" +
                "$filter=contains(fullname,'(sample)')and " +
                $"_parentcustomerid_value eq {contosoId}&" +
            ...
            JToken orderedResultsWithParams = Get("contacts?" +
                "$select=fullname,jobtitle,annualincome&" +
            ...
            JToken account1 = Get($"{account1Uri}?" +
                "$select=name&" +
            ...
            JToken contact2 = Get($"{contact1Uri}?$select=fullname,jobtitle,annualincome&" +
            ...
            JToken account2 = Get($"{account1Uri}?" +
                "$select=name&" +
            ...
            JToken account3 = Get($"{account1Uri}?$select=name&" +
                "$expand=primarycontactid($select=fullname,jobtitle,annualincome)," +
            ...
            JToken contactData = Get($"{account1Uri}/contact_customer_accounts?" +
            ...
            //Uri myUserQueryUri = PostCreate("userqueries", userQuery);
            //entityUris.Add(myUserQueryUri); //To delete later
            //                                //Retrieve the userqueryid
            //JToken myUserQueryId = Get($"{myUserQueryUri}/userqueryid")["value"];
            ////Use the query to return results:
            //JToken myUserQueryResults = Get($"contacts?userQuery={myUserQueryId}", formattedValueHeaders)["value"];
        ...
        private static void CreateRequiredRecords()
        {
            var account1 = new JObject {
                ...
                { "primarycontactid", new JObject{
                    ...
                    { "annualincome", 45000 },
                    { "emailaddress1", "Yvonne.McKay@notAReadAddress.org"},
                ...
                { "contact_customer_accounts", new JArray{
                        new JObject{
                                ...
                                { "annualincome", 52000},
                                { "emailaddress1", "Susanna.Stubberod@notemail.org"},
                                { "Contact_Tasks", new JArray{
                                new JObject{
            ...
            account1Uri = PostCreate("accounts", account1);
            entityUris.Add(account1Uri);
            contact1Uri = new Uri((Get($"{account1Uri}/primarycontactid/$ref"))["@odata.id"].ToString());
            entityUris.Add(contact1Uri);
            var contact_customer_accounts = Get($"{account1Uri}/contact_customer_accounts/$ref");
        ...
        private static void DeleteRequiredRecords(bool deleteCreatedRecords)
        {
        ...
        public static void Delete(string path, Dictionary<string, List<string>> headers = null)
        {
        ...
        public static async Task DeleteAsync(string path, Dictionary<string, List<string>> headers = null)
        {
        ...
        public static Uri PostCreate(string entitySetName, object body)
        {
            try
            {
        ...
        /// <param name="body">The JObject containing the data of the entity to create.</param>
        /// <returns>The Uri for the created entity record.</returns>
        public static async Task<Uri> PostCreateAsync(string entitySetName, object body)
        {
            try
            {
        ...
        public static JToken Get(string path, Dictionary<string, List<string>> headers = null)
        {
